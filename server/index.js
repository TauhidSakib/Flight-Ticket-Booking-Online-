...
app.get("/AvailableFlights", (req, res) => {
...
INNER JOIN 
    FlightStatus fs ON f.flightStatus_id = fs.flightStatus_id
INNER JOIN 
    Airplane a ON f.airplane_id = a.airplane_id
INNER JOIN
    Ticket t ON s.departure_time = t.departure_time
INNER JOIN
    Airports ap ON t.airport_code = ap.airport_code
WHERE 
    DATE(s.departure_time) = ? AND 
    DATE(s.arrival_time) = ?;
`;
...
// Route to fetch bookings for a specific client
app.get('/customerbookings/:id', (req, res) => {
    const clientId = req.params.id;

    // Query to fetch bookings for the given client ID
    const sqlQuery = `
    SELECT 
        b.booking_id, 
        b.client_id, 
        ...
        f.flight_no, 
        f.schedule_id, 
        f.airplane_id AS flight_airplane_id,
        s.departure_time,
        s.arrival_time,
        fs.flightStatus_id,
        fs.status,
    ...
    LEFT JOIN 
        Flight f ON b.airplane_id = f.airplane_id
    LEFT JOIN 
        Schedule s ON f.schedule_id = s.schedule_id
    LEFT JOIN 
        FlightStatus fs ON f.flightStatus_id = fs.flightStatus_id
    ...
    db.query(sqlQuery, [clientId], (err, results) => {
        if (err) {
...
// Server-side code
app.post("/bookTicketclient", (req, res) => {
    const { client_id, airplane_id } = req.body;
    
    // Insert booking into the database
    const sqlInsert = "INSERT INTO Bookings (client_id, airplane_id) VALUES (?, ?)";
    console.log(client_id, airplane_id)
    db.query(sqlInsert, [client_id, airplane_id], (err, result) => {
      if (err) {
