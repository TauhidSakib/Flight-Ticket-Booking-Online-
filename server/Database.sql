...
CREATE VIEW airport_view AS SELECT * FROM airports;
...
-- Alter FlightBooking table to add flight_no column
ALTER TABLE FlightBooking ADD COLUMN flight_no INT;

-- Delete row with fb_id = 65 from FlightBooking table
DELETE FROM FlightBooking WHERE fb_id = 65;

-- Update flight_no for fb_id = 45 in FlightBooking table
UPDATE FlightBooking SET flight_no = 40 WHERE fb_id = 45;

-- Select all rows from FlightBooking table
SELECT * FROM FlightBooking;

-- Delete row with fb_id = 47 from FlightBooking table
DELETE FROM FlightBooking WHERE fb_id = 47;

-- Create TempSeatGen table
CREATE TABLE TempSeatGen (
    gen_id INT PRIMARY KEY AUTO_INCREMENT,
    nm VARCHAR(3)
);

-- Insert data into TempSeatGen table
...
-- Select all rows from TempSeatGen table
SELECT * FROM TempSeatGen;

-- Select a random row from TempSeatGen table
SELECT nm FROM TempSeatGen ORDER BY RAND() LIMIT 1;

-- Select all rows from customer_review table
SELECT * FROM customer_review;

-- Insert a new row into customer_review table
...
DELIMITER $$
CREATE TRIGGER `airport_management`.`Ticket_AFTER_INSERT` AFTER INSERT ON `Ticket` FOR EACH ROW
BEGIN
    UPDATE Ticket SET seat_no = (SELECT nm FROM TempSeatGen ORDER BY RAND() LIMIT 1) WHERE seat_no IS NULL;
END$$
DELIMITER ;

-- Grant SELECT privilege on clients table to 'username'@'localhost'
GRANT SELECT ON airport_management.clients TO 'username'@'localhost';

-- Create trigger fill_booking
DELIMITER //
CREATE TRIGGER fill_booking AFTER INSERT ON Ticket
FOR EACH ROW
BEGIN
